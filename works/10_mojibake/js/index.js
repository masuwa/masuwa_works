...
(function (global, factory) {
  ...
  var ShuffleText = /** @class */ (function () {
      ...
      function ShuffleText(element) {
           ...
           */
          this.sourceRandomCharacter = '縺ｧｨｩ�ｦ譁ｭ代襍ｷｋｮ?繝舌ｪ~.ｮ□';
          /**
      ...
      ShuffleText.prototype._onInterval = function () {
          ...
          for (var i = 0; i < this._originalLength; i++) {
              ...
              str += this._originalStr.charAt(Math.floor(Math.random()*this._originalLength));
              }
              else {
...
function init() {
  ...
  for (var i = 0; i < elementList.length; i++) {
    elementList[i].dataset.index = i;
    effectList[i] = new ShuffleText(elementList[i]);
  }

  window.addEventListener('scroll', function(){
    for (var i = 0; i < elementList.length; i++) {
      var rect = elementList[i].getBoundingClientRect();//要素の絶対座標
